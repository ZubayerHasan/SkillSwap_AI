...
import { createBroadcast, fulfillBroadcast, getBroadcasts, getMyBroadcasts, getMyOffers } from "../../api/skillsApi";
...
const URGENCY_MAP = {
  1: { label: "Low", color: "text-text-muted bg-text-muted/10 border-text-muted/20" },
  2: { label: "Medium", color: "text-warning bg-warning/10 border-warning/20" },
  3: { label: "High", color: "text-danger bg-danger/10 border-danger/20" },
};
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Post a Skill Need">
      <div className="space-y-4">
            ...
            {[1, 2, 3].map((u) => (
              <button
                ...
                className={`flex-1 py-2 rounded-xl border text-sm font-semibold transition-all ${
                  form.urgency === u ? URGENCY_MAP[u].color + " border-current" : "border-border text-text-muted hover:border-brand/40"
                }`}
              >
                {URGENCY_MAP[u].label}
              </button>
        ...
        <div className="flex gap-3 pt-1">
          ...
          <Button
            id="post-broadcast-submit"
            className="flex-1"
            loading={mut.isPending}
            onClick={() => mut.mutate()}
            disabled={!form.skillName || !form.category}
          >
            Post Broadcast
          </Button>
        </div>
    ...
    <div className="card p-5 space-y-3 hover:border-brand/30 transition-all duration-200">
      ...
      <div className="flex items-center justify-between gap-2 pt-1">
        ...
        {isOwn ? (
          ...
          <Button size="sm" onClick={() => onHelp(b)}>
            I Can Help
          </Button>
        )}
      </div>
...
const DiscoveryPage = () => {
  ...
  const sendRequest = async () => {
    if (!reqForm.offeredSkillId) {
      toast.error("Select a skill you're offering");
      return;
    }
    if (!reqForm.proposedTime) {
      toast.error("Select a proposed time");
      return;
    }
    ...
    try {
      ...
      setReqForm({ offeredSkillId: "", proposedTime: "", message: "" });
    } catch (err) {
    ...
    <PageWrapper>
      <div className="space-y-6">
        ...
        <div className="flex gap-1 p-1 bg-background-card border border-border rounded-xl w-fit">
          {[
            ["discover", "🔍 Skills"],
            ["broadcasts", "📢 Community Needs"],
          ].map(([key, label]) => (
            <button
              key={key}
              onClick={() => setTab(key)}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                tab === key ? "bg-brand text-white shadow-glow" : "text-text-secondary hover:text-text-primary"
              }`}
            >
              {label}
            </button>
                    ...
                    {CATEGORIES.map((c) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full flex justify-between text-left px-3 py-2 text-sm rounded-lg transition-colors border ${
                            category === c
                              ? "bg-brand-dim/20 text-text-primary border-brand/20"
                              : "text-text-secondary border-transparent hover:bg-background-elevated hover:border-border"
                          }`}
                        >
                          <span>{c || "All"}</span>
                          {count ? <span className="text-text-muted">{count}</span> : null}
                        </button>
                      );
                    })}
                  </div>
                      ...
                      <div key={skill._id} className="card p-5 space-y-4 hover:border-brand/40 transition-all duration-200">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className="text-xs px-2 py-0.5 bg-background-elevated rounded-full text-text-muted">
                              {skill.category}
                            </span>
                        ...
                        {skill.user?._id !== user?._id && (
                          <div className="grid grid-cols-2 gap-2">
                            <Button
                              onClick={() => setRequestTarget({ skillOffer: skill, user: skill.user })}
                              variant="primary"
                              size="sm"
                              className="w-full"
                            >
                            ...
                            <Button
                              onClick={() => navigate("/chat", { state: { participantId: skill.user._id } })}
                              variant="ghost"
                              size="sm"
                              className="w-full"
                            >
                  ...
                  {myBroadcasts
                    ...
                    .map((b) => (
                      <BroadcastCard
                        ...
                        onFulfill={(id) => fulfillMut.mutate(id)}
                        onHelp={openBroadcastHelp}
                      />
                    ))}
                </div>
              </div>
        ...
        {requestTarget && (
          <div className="space-y-4">
            <div className="bg-background-elevated rounded-xl p-4 flex items-center justify-between gap-3">
              <div className="text-center">
                ...
                <select
                  ...
                  value={reqForm.offeredSkillId}
                  onChange={(e) => setReqForm((f) => ({ ...f, offeredSkillId: e.target.value }))}
                  required
                >
              ...
              <div className="text-center">
                ...
                <div className="px-3 py-2 bg-brand/10 border border-brand/20 rounded-lg text-sm text-brand font-medium">
                  {requestTarget.skillOffer.displayName}
                </div>
              </div>
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => setReqForm((f) => ({ ...f, proposedTime: e.target.value }))}
            ...
            <div className="flex gap-3">
              <Button variant="ghost" className="flex-1" onClick={() => setRequestTarget(null)}>
                Cancel
              </Button>
              <Button className="flex-1" loading={reqLoading} onClick={sendRequest}>
                Send Request
              </Button>
            </div>
          </div>
      ...
      <Modal isOpen={!!helpTarget} onClose={() => setHelpTarget(null)} title="Offer Help">
        {helpTarget && (
          <div className="space-y-4">
            ...
            <div className="flex gap-3">
              <Button variant="ghost" className="flex-1" onClick={() => setHelpTarget(null)}>
                Cancel
              </Button>
              <Button
                className="flex-1"
                loading={reqLoading}
                onClick={async () => {
                  if (!reqForm.offeredSkillId || !reqForm.proposedTime) {
                    toast.error("Select a skill and proposed time");
                    return;
                  }
                  ...
                  try {
                    await createExchangeRequest({
                      receiverId: helpTarget.userId?._id || helpTarget.userId,
                      offeredSkillId: reqForm.offeredSkillId,
                      requestedSkillId: helpTarget._id,
                      proposedTime: reqForm.proposedTime,
                      message: reqForm.message || `I can help you with ${helpTarget.displayName}!`,
                    });
